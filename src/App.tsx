...
export default function App() {
  ...
  const animatedScore = Math.min(fallbackScore, Math.round((progress / 100) * fallbackScore));
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setEmailError("Couldn't send email right now. Please try again in a moment.");
    }
  };
    ...
    <div className="min-h-screen bg-[#EDF6F9] text-slate-900">
      ...
      {(!PAYMENT_LINK_URL || !EMAIL_ENDPOINT_URL) && (
        <div className="bg-yellow-50 border-b border-yellow-200 text-yellow-800 text-sm">
          <div className="mx-auto max-w-6xl px-4 py-2">
            Running in <b>Test Mode</b>. Configure <code>VITE_PAYMENT_LINK_URL</code> and <code>VITE_EMAIL_ENDPOINT_URL</code> to go live.
          </div>
        </div>
      )}
      ...
      <header className="hidden md:block sticky top-0 z-20 backdrop-blur bg-white/70 border-b">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          ...
          <button onClick={runTest} disabled={loading || !url.trim()} className="rounded-xl px-4 py-2 text-white bg-[#006D77] hover:opacity-90 disabled:opacity-60">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-12 md:py-20 grid md:grid-cols-2 gap-8 md:gap-10 items-center">
          <div className="text-white">
            <h1 className="text-3xl md:text-5xl font-bold leading-tight">Full AI Website Audit in 1–2 Minutes</h1>
            <p className="mt-3 md:mt-4 text-base md:text-xl text-white/90">Instantly see what’s holding back your conversions and how to fix it — powered by 100+ CRO heuristics.</p>
            ...
            <div className="mt-3 md:mt-4 flex flex-wrap items-center gap-3 md:gap-4 text-white/80 text-sm">
              <div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-white/80"/>No registration</div>
              <div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-white/80"/>Results in 1–2 min</div>
              <div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-white/80"/>AI analysis</div>
            </div>
          </div>
          <div className="bg-white/80 rounded-2xl p-5 md:p-6 shadow-xl">
            {!loading && !showResults && (
              <div className="h-48 md:h-56 rounded-xl bg-slate-100 border border-slate-200 grid place-items-center text-slate-500">Hero Preview Placeholder</div>
            )}
            {loading && (
              <div className="h-48 md:h-56 rounded-xl bg-white/90 border border-slate-200 p-5 flex flex-col justify-center">
                ...
                <div className="mt-2 text-sm text-slate-600">Estimated score: {animatedScore}/100</div>
              </div>
            )}
            {showResults && (
              <div className="h-48 md:h-56 rounded-xl bg-slate-100 border border-slate-200 grid place-items-center text-slate-600">Analysis complete — see Preview below</div>
            )}
            <p className="mt-3 md:mt-4 text-slate-700 text-sm">This panel shows the current state of the audit (placeholder → analyzing → complete).</p>
          </div>
        </div>
      </section>

      {/* PREVIEW */}
      <section id="preview" ref={previewRef} className="mx-auto max-w-6xl px-3 md:px-4 py-10 md:py-16">
        {!showResults ? (
          <div className="rounded-2xl border bg-white p-5 text-slate-600 text-sm text-center">Run a test to see your live preview here.</div>
        ) : aiAudit ? (
          <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
            <div className="lg:col-span-1">
              <h2 className="text-2xl md:text-3xl font-semibold">Your Website Summary</h2>
              <p className="mt-2 text-slate-600">AI-generated score and top blockers.</p>
              <div className="mt-5 p-5 rounded-2xl border bg-white">
                <div className="text-sm text-slate-500">Conversion Readiness Score</div>
                <div className="mt-2 h-3 rounded-full bg-slate-200 overflow-hidden">
                  <div className="h-full bg-[#006D77]" style={{ width: `${aiAudit.score}%` }} />
                </div>
            ...
            <div className="lg:col-span-2 grid gap-4">
              {aiAudit.key_findings.map((f, i) => (
                <div key={i} className="p-5 rounded-2xl border bg-white">
                  <div className="flex items-center gap-2">
                    <span className={`h-2 w-2 rounded-full ${f.impact==='high'?'bg-red-500':f.impact==='medium'?'bg-amber-500':'bg-emerald-500'}`} />
          ...
          <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
            <div className="lg:col-span-1">
              <h2 className="text-2xl md:text-3xl font-semibold">Your Website Summary</h2>
              <p className="mt-2 text-slate-600">A fast, shareable overview with an AI-generated score and the top blockers dragging down conversions.</p>
              <div className="mt-5 p-5 rounded-2xl border bg-white">
                <div className="text-sm text-slate-500">Conversion Readiness Score</div>
                <div className="mt-2 h-3 rounded-full bg-slate-200 overflow-hidden">
                  <div className="h-full bg-[#006D77]" style={{ width: `${fallbackScore}%` }} />
                </div>
                <div className="mt-2 text-sm font-medium">{fallbackScore} / 100</div>
              </div>
            </div>
            <div className="lg:col-span-2 grid gap-4">
              {["Main CTA is below the fold","Mobile LCP is 3.5s (target <2.5s)","Low contrast on primary button","Unclear form labels","Missing trust signals above the fold"].map((t,i)=> (
                <div key={i} className="p-5 rounded-2xl border bg-white flex items-start gap-4">
                  <div className="mt-1 h-3 w-3 rounded-full bg-[#83C5BE]" />
                  <div>
                    <div className="font-medium">{t}</div>
                    <p className="text-sm text-slate-600 mt-1">Short rationale and what to do next. (This becomes a full annotated screenshot in the paid report.)</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="mx-auto max-w-6xl px-3 md:px-4 py-12 md:py-16">
        <div className="rounded-3xl border bg-white p-5 md:p-10 grid md:grid-cols-2 gap-6 md:gap-8 items-center">
          <div>
            <h3 className="text-xl md:text-2xl font-semibold">Get a Free PDF Summary</h3>
            <p className="mt-2 text-slate-600">Download your findings with annotated screenshots and quick fixes. No spam — unsubscribe anytime.</p>
          </div>
          <form onSubmit={handleEmailSubmit} className="flex w-full flex-col sm:flex-row gap-3">
            <input
          ...
          {(emailError || emailSubmitted) && (
            <div className="md:col-span-2">
              ...
              {emailSubmitted && (
                <p className="text-sm text-green-700">Check your inbox — we’ve sent a link to download your free PDF.</p>
              )}
            </div>
          )}
        </div>
      ...
      {/* FULL AUDIT OFFER */}
      <section id="pricing" className="mx-auto max-w-6xl px-3 md:px-4 py-12 md:py-16">
        <div className="rounded-3xl border bg-white p-5 md:p-10 grid md:grid-cols-2 gap-6 md:gap-8 items-center">
          <div>
            ...
            <div className="mt-6 flex gap-3">
              <button onClick={handleOrderFullAudit} className="rounded-xl px-5 py-3 bg-[#FFDDD2] text-slate-900 font-medium hover:opacity-90">Order Full Audit</button>
              <button onClick={handleSeeSample} className="rounded-xl px-5 py-3 border font-medium hover:bg-slate-50">See Sample Report</button>
            </div>
          </div>
          <div className="grid gap-3">
            <div className="h-40 md:h-44 rounded-2xl border bg-slate-50 grid place-items-center text-slate-500">Report preview placeholder</div>
            <div className="h-16 md:h-20 rounded-2xl border bg-slate-50 grid place-items-center text-slate-500">Metrics / badges</div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-3 md:px-4 py-12 md:py-16">
        <div className="grid md:grid-cols-2 gap-6 md:gap-8">
          <div className="rounded-3xl border bg-white p-5 md:p-6">
            <h4 className="text-lg md:text-xl font-semibold">Free Test — Benefits</h4>
          ...
          <div className="rounded-3xl border bg-white p-5 md:p-6">
            <h4 className="text-lg md:text-xl font-semibold">Full Audit — Benefits</h4>
            <ul className="mt-3 space-y-2 text-slate-700 text-sm md:text-base">
      ...
      <section className="mx-auto max-w-6xl px-3 md:px-4 py-12 md:py-16">
        <div className="rounded-3xl border bg-white p-5 md:p-10 grid md:grid-cols-3 gap-6 md:gap-8 items-center">
          <div className="md:col-span-2">
            <h3 className="text-2xl font-semibold">Case Study: +21% Conversions in 30 Days</h3>
            <p className="mt-2 text-slate-600">Before: CTA hidden below the fold, slow load times. After: CTA above the fold, load time < 2.0s. Result: more sign‑ups and lower CPA.</p>
          </div>
          <div className="h-28 md:h-32 rounded-2xl border bg-slate-50 grid place-items-center text-slate-500">Before/After chart</div>
        </div>
      </section>

      {/* FAQ */}
      <section id="faq" className="mx-auto max-w-6xl px-3 md:px-4 py-12 md:py-16">
        <h3 className="text-2xl font-semibold">FAQ</h3>
        <div className="mt-6 grid md:grid-cols-2 gap-6">
          {[
            ["Does AI make mistakes?","The analysis is based on measurable data and CRO standards, but human validation is always possible."],
