...
/**
 * Holbox AI – Landing (single-file React + Tailwind)
 * Next step: live endpoints + graceful fallbacks (no payment processor yet).
 *
 * Configure via .env (Vite) or inline constants below.
 * - VITE_PAYMENT_LINK_URL: Stripe (or alt) Payment Link for $50 Full Audit
 * - VITE_EMAIL_ENDPOINT_URL: Formspree/Web3Forms/Basin POST URL
 * - VITE_SALES_EMAIL: mailbox for fallback mailto order
 */
...
export default function App() {
  ...
  // Animated score target (what the AI would compute)
  const targetScore = 72;
  const animatedScore = Math.min(
    targetScore,
    Math.round((progress / 100) * targetScore)
  );
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    // If no endpoint configured yet, do a graceful success (mock) and hint
    ...
    try {
      const payload = {
        email,
        site: url || "",
        score: showResults ? targetScore : undefined,
  ...
  const handleOrderFullAudit = () => {
    ...
    // Fallback: open mailto order if no processor yet
