...
export default function Landing({
  ...
  const [data, setData] = useState<any | null>(null); // ✅ reālie dati
  const [err, setErr] = useState<string | null>(null); // ✅ kļūdas ziņojums
  const previewRef = useRef<HTMLDivElement | null>(null);
  const progressTimer = useRef<number | null>(null);

  // --- Demo “satura” saraksti atstājam, jo tie ir UI elements, bet tie nav saistīti ar fetch ---
  ...
  // ✅ vienkāršs “score” no reālajiem datiem (minimāli invazīvi)
  function computeScores(d: any | null) {
    ...
    if (imgs > 0) {
      ...
      score += Math.round(10 * altRatio); // līdz +10
    }
    ...
    // saturam/struktūrai relatīvas atvasinātas sadaļas
    let structure = 30;
    ...
    content += Math.min(15, (d.images?.total ?? 0) > 0 ? 10 : 0);
    content += Math.min(
      10,
  ...
  // ===== RUN TEST + reāls fetch + animēts progress =====
  useEffect(() => {
    return () => {
  ...
  function startProgress() {
    ...
    const step = () => {
      setProgress((p) => {
        if (!loading) return p; // apstādinām, kad fetch beidzies
        const next = p + Math.random() * 6 + 1; // plūdeni, kā Full report
        return Math.min(next, 90); // līdz ~90%, kamēr gaidām atbildi
      });
  ...
  const demoScore = scores.overall; // ✅ vairs nav demo – no datiem
  const structurePct = scores.structure;
  const contentPct = scores.content;
  ...
  return (
    <div className="min-h-screen bg-[#EDF6F9] text-slate-900">
      ...
      >
              ...
              <div className="rounded-2xl border bg-white p-5 md:p-6">
                ...
                {data && (
                  <div className="mt-4 text-xs text-slate-600">
                    <div>
                      <b>URL:</b> {data.finalUrl || data.url}
                    </div>
                    <div>
                      <b>HTTP:</b> {data.httpStatus}
                    </div>
                    <div>
                      <b>Lang:</b> {data.meta?.lang ?? "—"}
                    </div>
                  </div>
                )}
              </div>
        ...
        {/* Kļūdas ziņojums */}
        {err && (
          <div className="mt-4 rounded-xl border bg-rose-50 text-rose-800 p-4 text-sm">
            {err}
          </div>
        )}
      </section>
